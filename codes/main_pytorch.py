...
def train_model(network_data, feature_dic):
    ...
    for epoch in range(epochs):
        ...
        logger.info(f"valid auc: {np.mean(valid_aucs)}")
        logger.info(f"valid pr: {np.mean(valid_prs)}")
        logger.info(f"valid f1: {np.mean(valid_f1s)}")
        ...
        for k, v in final_model.items():
            final_model_0 = v
            break
        new_final_model_0 = dict(map(lambda x: (x[0], x[1].tolist()), final_model_0.items()))

        save_as_json(new_final_model_0, f'./data/results/final_model_epoch_{epoch}.json')
            # with open(f'./data/results/final_model_epoch_{epoch}.bin', 'w', encoding='utf-8') as fh:
            #     fh.write(str(final_model))
            # print(f'final_model_epoch_{epoch}.json saved at ./data/results/')
        print(f'final_model_epoch_{epoch}.json saved at ./data/results/')
...
if __name__ == "__main__":
    ...
    logger = Logger('gatne_all_data_epoch_1').logger
    logger.info('Begin...')
    start = time.time()
    ...
    logger.info(f"Overall ROC-AUC: {average_auc}" )
    logger.info(f"Overall PR-AUC: {average_pr}")
    logger.info(f"Overall F1: {average_f1}")

    logger.info('Done.')
    end = time.time()
    logger.info(f'The whole process cost {end-start} seconds.')
